          ...
          <div className='absolute inset-0 flex items-end text-white px-6 py-12 md:py-36'>
            <div className='flex flex-col'>
                ...
                <ul className='flex gap-4 items-center text-lg md:text-2xl mt-2 md:mt-4'>
                    ...
                    <li className={`flex items-center gap-4 ${currentLocation.pathname.includes("/room-details") ? "block" : "hidden"}`}><img src={LocationImg} alt="P0inter" className='w-4 h-4 md:w-6 md:h-6 object-contain shrink-0 invert' /><Link to={'/our-rooms'} className="cursor-pointer">Rooms</Link><img src={LocationImg} alt="P0inter" className='w-4 h-4 md:w-6 md:h-6 object-contain shrink-0 invert' /><Link to={'/room-details'} className={`cursor-pointer ${currentLocation.pathname.includes("/room-details") ? "text-[#FFD400] tracking-wide" : "text-white"}`}>Beachfront Bungalow</Link></li>
                </ul>
            </div>
          </div>
        </div>
